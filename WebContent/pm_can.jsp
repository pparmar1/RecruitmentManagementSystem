...
<%String username=(String)session.getAttribute("username");%>
     ...
     <form class="form-signin" action="ViewRegisteredCandidate" method="post">
        <h2 class="form-signin-heading">Registered Candidates </h2>
 	<%
		Connection con = null;
	...
	%>
 <button class="btn btn-lg btn-primary btn-block" style="max-width:200px ; margin-left:70px "type="submit">Submit</button>
         	 </form>   
         </div>
	...
	String JOB_ID=(String)session.getAttribute("JOB_ID");
			List<CandidateDetails> list = r.displayCan(JOB_ID);%>
		<% 	for(CandidateDetails rs : list ){
			%>	
				<tr><td><%=rs.getCID()%></td>
				<td><%=rs.getName() %></td>
				<td><%=rs.getContact_no() %></td>
				<td><%=rs.getEmail() %></td>
				</tr>
			<% }%>
			</tbody>
